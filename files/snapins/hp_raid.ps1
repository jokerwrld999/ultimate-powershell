...
$hpSrcUnzipPath = "$hpDriverTempDir\raid.exe"
$hpDestUnzipPath = "$hpDriverTempDir\Raid"
$hpDriverSrc = "$hpDriverTempDir\Raid\Setup.exe"
...
if (!(Test-Path -Path $hpDriverSrc)) {
  ...
  Write-Host "####### Downloading HP Raid Driver... #######" -ForegroundColor Blue
...
if (!(Test-Path -Path $hpDriverSrc)) {
  ...
  Write-Host "####### Extracting HP Raid Driver... #######" -ForegroundColor Blue
  Start-Process $7zipExe -ArgumentList "x $hpSrcUnzipPath `"-o$($hpDestUnzipPath)`" -y -bso0 -bd" -NoNewWindow -Wait
}

Write-Host "####### Installing HP Raid Driver... #######" -ForegroundColor Blue
